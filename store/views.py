...
feature_list = np.array(pickle.load(open('/fashion-recommender-system/embeddings.pkl', 'rb')))
filenames = pickle.load(open('/fashion-recommender-system/filenames.pkl', 'rb'))
...
def get_product_id_from_image_url(image_url):
    try:
        # Tìm sản phẩm có image_url tương ứng
        product = Product.objects.get(image_url=image_url)
        # Trả về id của sản phẩm
        return product.id
    except Product.DoesNotExist:
        # Nếu không tìm thấy sản phẩm, trả về None hoặc một giá trị mặc định khác tùy vào yêu cầu của bạn
        return None
